...
## Full Local Installation Guidance

### PHP Setup
- Download PHP from https://windows.php.net/download/ (choose the latest "Non Thread Safe" .zip for your system).
- Extract to a folder (e.g., `C:\php`).
- Add the PHP folder to your system PATH.
- Copy `php.ini-development` to `php.ini` and enable required extensions:
  - Uncomment `extension=openssl`
  - Uncomment/add `extension=fileinfo`
  - Download the correct `php_mongodb.dll` from https://pecl.php.net/package/mongodb (matching your PHP version, architecture, and TS/NTS).
  - Place `php_mongodb.dll` in your PHP `ext` directory and add `extension=php_mongodb.dll` to `php.ini`.
...
- Download Composer installer:
  ...
  ```
- Use Composer via `php composer.phar install`.
...
- (Optional) Run migrations: `php artisan migrate`
- Start server: `php artisan serve`

Troubleshooting:
- If you see missing extension errors, ensure all required DLLs are present and enabled in `php.ini`.
- Restart your terminal after changes to PATH or `php.ini`.
## Installation
